...
public class LinkedHashSetTask {
    public static void main(String[] args) {
        ...
        for (int i = 0; i<testCases;i++){
            ...
            for (int j=0;j<numberOfQueries;j++){
                ...
                executeUserQuery(userQuery,linkedHashSet);
    ...
    public static void executeUserQuery(String userQuery,LinkedHashSet<Integer> linkedHashSet){
        char selectedOption =userQuery.charAt(0);
        String element = null;
        switch (selectedOption){
            case 'a':
                element=userQuery.substring(2); // substring from the third letter , after the a and the space
            ...
            case 'b':
                ...
                break;
            case 'c':
                element=userQuery.substring(2);   // substring from the third letter , after the c and the space
                linkedHashSet.remove(Integer.parseInt(element));
                break;
            case 'd':
                element=userQuery.substring(2);   // substring from the third letter , after the d and the space
